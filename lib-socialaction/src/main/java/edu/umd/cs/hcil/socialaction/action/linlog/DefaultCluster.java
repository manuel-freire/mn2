 ...
 * (ca.ubc.cs.smallworld.types.DefaultCluster) to work with Prefuse Beta
 * 
 * @author Stephen Frowe Ingram
 * @author Adam Perer
 */
public class DefaultCluster extends TableNode implements Cluster {
	 ...
	 * @return
	 */
	// public static Graph generateTestSet( ) {
	//
	// Graph graph = new Graph( );
	// Cluster c1 = new DefaultCluster( 200, 300, 50.f );
	// Cluster c2 = new DefaultCluster( 400, 300, 50.f );
	// // graph.addNode( new DefaultCluster( 300, 300,
	// c1.getRadius()+c2.getRadius(),c1,c2 ) );
	// graph.addEdge( new Edge(c1, c2) );
	// Cluster c3 = new DefaultCluster( 100, 200, 50.f );
	// Cluster c4 = new DefaultCluster( 300, 300,
	// c1.getRadius()+c2.getRadius(),c1,c2 );
	// graph.addNode( new DefaultCluster( 300, 300,
	// c3.getRadius()+c4.getRadius(),c3,c4 ) );
	// return graph;
	// }
	...
	public DefaultCluster(float x, float y, float radius, Cluster child_1,
			Cluster child_2, double distance) {
		...
		double tempw = Math.max((x + radius) - tempx, (child_1.getBounds()
				.getX() + child_1.getBounds().getWidth())
				- tempx);
		double temph = Math.max((y + radius) - tempy, (child_1.getBounds()
				.getY() + child_1.getBounds().getHeight())
				- tempy);
		tempw = Math.max(tempw, (child_2.getBounds().getX() + child_2
				.getBounds().getWidth())
				- tempx);
		temph = Math.max(temph, (child_2.getBounds().getY() + child_2
				.getBounds().getHeight())
				- tempy);
	...
	public DefaultCluster(float x, float y, float radius, Cluster child_1,
		...
		if (child_1 != null) {
			...
			if (child_2 != null) {
				child_2.setParent(this);
				m_children.add(child_2);
				m_distance = (float) child_1.getCenter().distance(
						child_2.getCenter());
				...
				tempx = Math.min(tempx, child_2.getBounds().getX());
				tempy = Math.min(tempy, child_2.getBounds().getY());
				double tempw = Math.max((x + radius) - tempx, (child_1
						.getBounds().getX() + child_1.getBounds().getWidth())
						- tempx);
				double temph = Math.max((y + radius) - tempy, (child_1
						.getBounds().getY() + child_1.getBounds().getHeight())
						- tempy);
				tempw = Math.max(tempw, (child_2.getBounds().getX() + child_2
						.getBounds().getWidth())
						- tempx);
			...
			double tempw = Math.max((x + radius) - tempx, (child_2.getBounds()
					.getX() + child_2.getBounds().getWidth())
					- tempx);
			double temph = Math.max((y + radius) - tempy, (child_2.getBounds()
					.getY() + child_2.getBounds().getHeight())
					- tempy);
			m_bounds = new Rectangle2D.Double(tempx, tempy, tempw, temph);
		} else {
			// m_children = null;
			m_bounds = new Rectangle2D.Double(x - radius, y - radius,
	...
	private int getHeight_helper(Cluster child) {
		...
		if (iter != null) {
			while (iter.hasNext()) {
				// System.out.println("id="+getAttribute("id"));
				maxheight = Math.max(getHeight_helper((Cluster) iter.next()),
						maxheight);
			}
			return maxheight + 1;
		} else {
