 ...
 * 
 * <p>
 * When created using the default constructor, the renderer attempts to use text from the "label" field. To use a
 * different field, use the appropriate constructor or use the {@link #setTextField(String)} method. To perform custom
 * String selection, subclass this Renderer and override the {@link #getText(VisualItem)} method. When the text field is
 * <code>null</code>, no text label will be shown. Labels can span multiple lines of text, determined by the presence of
 * newline characters ('\n') within the text string.
 * </p>
 * 
 * <p>
 * By default, no image is shown. To show an image, the image field needs to be set, either using the appropriate
 * constructor or the {@link #setImageField(String)} method. The value of the image field should be a text string
 * indicating the location of the image file to use. The string should be either a URL, a file located on the current
 * classpath, or a file on the local filesystem. If found, the image will be managed internally by an
 * {@link ImageFactory} instance, which maintains a cache of loaded images.
 * </p>
 * 
 * <p>
 * The position of the image relative to text can be set using the {@link #setImagePosition(int)} method. Images can be
 * placed to the left, right, above, or below the text. The horizontal and vertical alignments of either the text or the
 * image can be set explicitly using the appropriate methods of this class (e.g.,
 * {@link #setHorizontalTextAlignment(int)}). By default, both the text and images are centered along both the
 * horizontal and vertical directions.
 * </p>
 * 
 * @author <a href="http://jheer.org">jeffrey heer</a>
 */
public class SelectedLabelRenderer extends AbstractShapeRenderer {

	// private boolean SHOW_PICTURES = false;
	// private boolean SHOW_TEXT = true;
	// private boolean HIDE_FILTERED = true;
	// private boolean SHOW_HIGHLIGHT = true;
	...
	private boolean m_showPictures = false;
	...
	private boolean m_showFiltered = true;

	public void setShowFiltered(boolean show) {
		m_showFiltered = show;
	}

	public boolean getShowFiltered() {
		return m_showFiltered;
	}

	private boolean m_showHighlight = true;
	...
	/**
	 * Create a new LabelRenderer. By default the field "label" is used as the field name for looking up text, and no
	 * image is used.
	 */
	public SelectedLabelRenderer(SocialAction app) {
		m_app = app;
	}
	 ...
	 */
	public SelectedLabelRenderer(String textField) {
		this.setTextField(textField);
	}

	/**
	 * Create a new LabelRenderer. Draws a text label using the given text data field, and draws the image at the
	 * location reported by the given image data field.
	 * 
	 * @param textField
	 *            the data field for the text label
	 * @param imageField
	 *            the data field for the image location. This value in the data field should be a URL, a file within the
	 *            current classpath, a file on the filesystem, or null for no image. If the <code>imageField</code>
	 *            parameter is null, no images at all will be drawn.
	 */
	public SelectedLabelRenderer(String textField, String imageField) {
	...
	/**
	 * Rounds the corners of the bounding rectangle in which the text string is rendered. This will only be seen if
	 * either the stroke or fill color is non-transparent.
	 * 
	 * @param arcWidth
	...
	/**
	 * Sets the maximum width that should be allowed of the text label. A value of -1 specifies no limit (this is the
	 * default).
	 * 
	 * @param maxWidth
	...
	/**
	 * Get the data field for image locations. The value stored in the data field should be a URL, a file within the
	 * current classpath, a file on the filesystem, or null for no image.
	 * 
	...
	/**
	 * Set the data field for image locations. The value stored in the data field should be a URL, a file within the
	 * current classpath, a file on the filesystem, or null for no image. If the <code>imageField</code> parameter is
	 * null, no images at all will be drawn.
	 * 
	 * @param imageField
	...
	/**
	 * Sets the maximum image dimensions, used to control scaling of loaded images. This scaling is enforced immediately
	 * upon loading of the image.
	 * 
	 * @param width
	...
	/**
	 * Returns a location string for the image to draw. Subclasses can override this class to perform custom image
	 * selection beyond looking up the value from a data field.
	 * 
	 * @param item
	...
	private String computeTextDimensions(VisualItem item, String text, double size) {
		...
		boolean as = (rangeSliderSet.containsTuple(item.getSourceTuple()));
		if (as || (m_app.getNodeColorField() == null))
			m_font = FontLib.getFont(SocialAction.DEFAULT_FONTNAME, Font.PLAIN, m_app.getFontSize());

		else if (!as && (!getShowFiltered())) {
			m_font = FontLib.getFont(SocialAction.DEFAULT_FONTNAME, Font.PLAIN, 1);
		} else
			m_font = FontLib.getFont(SocialAction.DEFAULT_FONTNAME, Font.PLAIN, 6);
	...
	protected Shape getRawShape(VisualItem item) {
		...
		// m_font = FontLib.getFont("Tahoma",Font.PLAIN,m_app.getFontSize());

		// System.out.println("RAWSHAPE: " + m_app.getFontSize());
		m_text = getText(item);

		// m_text = StringW.wordWrap(m_text,20, "\n", "-");
		...
		// if (item.isInGroup(m_app.getRangeBinding().g.getPredicate().toString()))
		// setMaxTextWidth(1);

		// FocusSet alphaSliderSet =
		// item.getItemRegistry().getFocusManager().getFocusSet(SocialNetworkExplorer.ALPHASLIDER_KEY);
		TupleSet sliderSet = m_app.getVisualization().getFocusGroup(SocialAction.RANGE_NODE_SLIDER_GROUP);

		boolean as = sliderSet.containsTuple(item.getSourceTuple()); // item.isInGroup(SocialAction.RANGE_SLIDER_GROUP);

		// System.out.println(as2 + " " + sliderSet.getTupleCount());

		if (m_app.getNodeColorField() == null)
			as = true;
		...
		// String fontname = SocialAction.DEFAULT_FONTNAME;

		if (!as) {// !as) {
			setMaxTextWidth(1);
			setMaxImageDimensions(1, 1);
			iw = 0;
			ih = 0;

			// s = "";
			// m_font = FontLib.getFont(fontname,Font.PLAIN,6);
		}
		// else if (item.i)
		// {
		// setMaxImageDimensions(1,1);
		// }
			...
			// setMaxImageDimensions(25,25);
			setMaxTextWidth(m_app.getLabelSize());

			// if (fadeIsolates) {
			// NodeItem nodeItem = (NodeItem) item;
			// if (nodeItem.getEdgeCount() == 0)
			// m_font = FontLib.getFont(fontname,Font.PLAIN,12);
			//
			// }
	...
	public void render(Graphics2D g, VisualItem item) {
		...
		if (text != null && ColorLib.alpha(textColor) > 0) {
			...
			boolean as2 = (rangeSliderSet.containsTuple(item.getSourceTuple()));
			if (as2 || (m_app.getNodeColorField() == null))
				m_font = FontLib.getFont(SocialAction.DEFAULT_FONTNAME, Font.PLAIN, m_app.getFontSize());
			else
				m_font = FontLib.getFont(SocialAction.DEFAULT_FONTNAME, Font.PLAIN, 6);
			...
			if (as2 || !getShowFiltered() || (m_app.getNodeColorField() == null))
	...
	/**
	 * Get the horizontal text alignment within the layout. One of {@link prefuse.Constants#LEFT},
	 * {@link prefuse.Constants#RIGHT}, or {@link prefuse.Constants#CENTER}. The default is centered text.
	 * 
	...
	/**
	 * Set the horizontal text alignment within the layout. One of {@link prefuse.Constants#LEFT},
	 * {@link prefuse.Constants#RIGHT}, or {@link prefuse.Constants#CENTER}. The default is centered text.
	 * 
	 * @param halign
	...
	/**
	 * Get the vertical text alignment within the layout. One of {@link prefuse.Constants#TOP},
	 * {@link prefuse.Constants#BOTTOM}, or {@link prefuse.Constants#CENTER}. The default is centered text.
	 * 
	...
	/**
	 * Set the vertical text alignment within the layout. One of {@link prefuse.Constants#TOP},
	 * {@link prefuse.Constants#BOTTOM}, or {@link prefuse.Constants#CENTER}. The default is centered text.
	 * 
	 * @param valign
	...
	/**
	 * Get the horizontal image alignment within the layout. One of {@link prefuse.Constants#LEFT},
	 * {@link prefuse.Constants#RIGHT}, or {@link prefuse.Constants#CENTER}. The default is a centered image.
	 * 
	...
	/**
	 * Set the horizontal image alignment within the layout. One of {@link prefuse.Constants#LEFT},
	 * {@link prefuse.Constants#RIGHT}, or {@link prefuse.Constants#CENTER}. The default is a centered image.
	 * 
	 * @param halign
	...
	/**
	 * Get the vertical image alignment within the layout. One of {@link prefuse.Constants#TOP},
	 * {@link prefuse.Constants#BOTTOM}, or {@link prefuse.Constants#CENTER}. The default is a centered image.
	 * 
	...
	/**
	 * Set the vertical image alignment within the layout. One of {@link prefuse.Constants#TOP},
	 * {@link prefuse.Constants#BOTTOM}, or {@link prefuse.Constants#CENTER}. The default is a centered image.
	 * 
	 * @param valign
	...
	/**
	 * Get the image position, determining where the image is placed with respect to the text. One of
	 * {@link Constants#LEFT}, {@link Constants#RIGHT}, {@link Constants#TOP}, or {@link Constants#BOTTOM}. The default
	 * is left.
	 * 
	...
	/**
	 * Set the image position, determining where the image is placed with respect to the text. One of
	 * {@link Constants#LEFT}, {@link Constants#RIGHT}, {@link Constants#TOP}, or {@link Constants#BOTTOM}. The default
	 * is left.
	 * 
	 * @param pos
	 ...
	 * 
	 * @param align
	 *            the horizontal alignment, one of {@link prefuse.Constants#LEFT}, {@link prefuse.Constants#RIGHT}, or
	 *            {@link prefuse.Constants#CENTER}.
	 */
	 ...
	 * 
	 * @param align
	 *            the vertical alignment, one of {@link prefuse.Constants#TOP}, {@link prefuse.Constants#BOTTOM}, or
	 *            {@link prefuse.Constants#CENTER}.
	 */
	...
	/**
	 * Returns the amount of padding in pixels between the content and the border of this item along the horizontal
	 * dimension.
	 * 
	...
	/**
	 * Sets the amount of padding in pixels between the content and the border of this item along the horizontal
	 * dimension.
	 * 
	 * @param xpad
	...
	/**
	 * Returns the amount of padding in pixels between the content and the border of this item along the vertical
	 * dimension.
	 * 
	...
	/**
	 * Sets the amount of padding in pixels between the content and the border of this item along the vertical
	 * dimension.
	 * 
	 * @param ypad
