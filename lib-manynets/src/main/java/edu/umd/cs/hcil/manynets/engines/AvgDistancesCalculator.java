 ...
 */
public class AvgDistancesCalculator extends AbstractCalculator {

    public static Stat avgDistancesG = new Stat("Avg distances",
                "Average distances from each vertex to all others",
                Distribution.class, Level.Network, Level.Network);
    public static Stat avgDistancesN = new Stat("Avg distances",
                "Average distances from each vertex to all others",
                Double.TYPE, Level.Node, Level.Network);

    public AvgDistancesCalculator() {
        super(new Stat[] {avgDistancesG, avgDistancesN}, "O(V^3)");
    }
    ...
    public void calculate(TableWrapper tw, int gid) {
        ...
        checkAddStat(avgDistancesG, tw);
        checkAddStat(avgDistancesN, pg.getNodeTable());
        Table t = tw.getTable();
        Table nt = g.getNodeTable();

        Map<Node,Double> m = GraphStatistics.averageDistances(g);
        ...
        for (Entry<Node,Double> e : m.entrySet()) {
            ...
            nt.set(e.getKey().getRow(), avgDistancesN.getName(), e.getValue());
            dhm.addValue((float)(double)e.getValue(),
                pg.getNodeTable().getRef(id));
        }
        t.set(r, avgDistancesG.getName(), dhm);
    }
}
